...
| A6 | Binary-structure tree | Missing config.rs, diagnose/, index/{walk,sync,secrets}, stubs, tei/, util/; lists nonexistent mcp/ and parse/{openapi,helm,markdown} — as of 2026-06-18 only `parse/helm` is still nonexistent (`openapi`/`markdown` now exist), and the `parse/` subtree was rewritten in the plan that day; the rest of the tree rewrite stays open (don't itemize — rewrite the tree) | plan 580–627 |
