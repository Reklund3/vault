...
```
Step 0   Confirm embedding approach + dims        (curl mlx_lm.server, update vault.toml)
...
Step 8   src/embed/                                (embeddings)
Step 9   src/index/classify.rs                     (Gemma classification)
Step 10  src/retrieve/router.rs                    (Gemma query plan)
...
1. **Check Step 0 status.**
   - Open `vault.toml` if it exists at project root or `~/.vault/vault.toml`. Look for `[mlx]` section with `embed_dims = N` where N > 0.
   - If `embed_dims` is missing, 0, or commented — Step 0 is **not done**, and Step 1 must not start.
...
4. **Surface blockers.** Common ones:
   ...
   - `Cargo.toml` missing critical deps (`rusqlite` with `bundled`, `sqlite-vec`, embedding crate, `serde`, `tiktoken-rs`)
...
Step 0   Embedding dimensions       ⚠ NOT CONFIRMED — vault.toml missing [mlx].embed_dims
...
Next action: Confirm Gemma 4 embedding output dimensions
  $ curl http://localhost:8080/v1/embeddings \
      -H "Content-Type: application/json" \
      -d '{"model": "<tag>", "input": "test"}'
  Then set [mlx].embed_dims in vault.toml.

Open Cargo.toml dependencies needed before Step 1: rusqlite (bundled), sqlite-vec,
fastembed (or chosen embed crate), tiktoken-rs, serde, serde_json, tokio (for hook IO).
```
